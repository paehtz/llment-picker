...
| **Hold Shift** while an element is outlined, then **Shift+↑/↓** or **Shift+click** on another sibling | **range of adjacent siblings** (several sections of a page): Shift freezes the outlined element as anchor, arrows or Shift+click move the focus, everything in between is selected and the outline grows; a plain click copies the range – always with a screenshot of the whole range, Ctrl adds one HTML file with all elements. Selector line becomes a selector list (`#a, #b, #c`) plus a line "Range: 3 adjacent siblings, from … to …". Tap Shift without choosing anything and the hover is free again |
