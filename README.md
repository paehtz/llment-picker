...
- **It sees nothing until you click.** Only `activeTab` – no host permissions, no content script running on every page. Chrome shows no "read and change all your data" warning. That also draws a line: `activeTab` covers the page itself, not frames a page embeds from another provider (HubSpot or Typeform forms, payment fields, consent banners). Elements inside such a frame cannot be picked; the picker says so and lets you pick the frame element instead, naming its URL. If you need them, the settings hold an opt-in that asks the browser for access to all sites, revocable there at any time.
...
| **R** (arms the record chip, like holding Alt or Ctrl), then click or release a lasso – or icon menu "record interaction", or right-click → "Record interaction here" | **interaction recording**, 5 s: hover, click, press keys – the picker logs what the page does (`:hover`/`:focus`/`:active` rules that apply, declared transitions and animations with warnings such as `transition: all` or animated layout properties, a computed-style diff 450 ms after the pointer enters, DOM changes, running animations from `getAnimations()` with duration and easing, frame timing with long frames and `long-animation-frame` entries) and takes a frame every 0.55 s; the frames become a contact sheet with timestamps and the pointer as a blue dot. For "the hover stutters", "the popup sits wrong": a video is nothing an LLM can read, frames plus data are. Esc ends early. Ctrl adds the HTML file of the target container; screenshot and recording exclude each other (arming one disarms the other) |
| **Hold Shift** while an element is outlined, then **Shift+↑/↓** or **Shift+click** | **multi-selection**: Shift freezes the outlined element as anchor and shows a dashed preview under the mouse. Shift+click on a sibling of the anchor selects the range up to it; Shift+click on any other element adds it individually (again removes it). The outermost selected element that contains other selected elements becomes the **context**: screenshot and HTML show the container, the selector line names the inner elements (all of them, including intermediate levels), a line "Context: … (screenshot and HTML show this container; the reference is the selector line above)" ties both together, and one line per inner element gives its position in the screenshot in image pixels ("In screenshot: <selector> @ x,y · w×h px") – the screenshot itself stays untouched, so it still shows the frontend as it is. For "here is the section, and I mean this button". Shift+↑/↓ walks to the previous/next sibling; when there is none, the selection jumps to the parent and continues outward from there. A plain click or **Enter** (also while Shift is still held) copies the selection – always with a screenshot of everything, Ctrl adds one HTML file with all elements. The selector line becomes a selector list (`#a, #b, #c`) plus "Range: 3 adjacent siblings, from … to …" or "Selection: N elements (not all adjacent)". Tap Shift without choosing anything and the hover is free again |
...
| **Tap Alt, Ctrl or R** (press and release, no click) | arms that mode: the chip stays lit, the next click or lasso triggers it – no need to hold the key while aiming. Tap again to disarm; holding the key (longer than 400 ms) is "on" while held and "off" after release |
...
Above the outline four symbols (`</>`, camera, record dot, dashed box) show what the click will add; they light up while the key is held or the lasso is being drawn and carry the hints ("Ctrl HTML", "Alt Screenshot", "Drag region", "R Record") when the element is wide enough for the whole row, symbols only otherwise – permanently, switchable off in the settings. They are deliberately not clickable: the mouse would have to cross other elements to reach them and the outline would jump.
...
- Optional permission: `optional_host_permissions: <all_urls>`, not granted on install. The settings switch "Also allow elements inside embedded frames from other sites" requests it via `permissions.request` and gives it back via `permissions.remove`; without it, injection into cross-origin frames fails silently, which the picker now reports instead of showing a dead zone (frames announce themselves to the top frame via `postMessage`, so the main frame knows which ones it reached). A pointer over a cross-origin frame delivers no events to the page at all, so such a frame is reached with Shift+arrow from a neighbouring element and copied with Enter; a frame holding the focus is blurred when the picker starts, otherwise every modifier key goes into the frame instead of to the picker.
...
- HTML file: `downloads.download` from the background script (blob URL in Firefox, `data:` URL in Chrome's service worker), filename `YYYY-MM-DD_HHMM_<host>_<id-or-class>.html`, absolute path from `downloads.search`. CSS context (guiding principle: drop what an agent can derive, keep what deviates or carries meaning): effective values only where they differ from the initial value (a fixed core – display, position, size, margin, padding – always), flex/grid properties only with a matching `display`; matching rules with origin (file, `inline <style>` marked as "loads last", Theme/Plugin/Core by path heuristic), specificity, media-query state and – from a cascade pass over the matched declarations – which properties a rule loses to a later or stronger one; universal `*` rules are skipped; `var(--x)` is shown with its resolved value; the direct children with box model and their rules (the levers of spacing tasks sit one level below), a spacing chain (margins/paddings of the previous and next sibling, gaps and margins of the children), the parent, and ancestors with deviations only (`overflow`, `position`, `transform`, `max-width` …); `::before`/`::after` with content (chevrons, lines, icons) and the element's own state rules (`:hover`, `:focus`, `:active`) that are not active at the time of capture; cross-origin stylesheets cannot be read and are listed as such. The analysis always targets the picked element(s): with a multi-selection each element gets its own block, and when the markup shown is a context container (Shift range with an outer element), the picked elements are marked `data-llment-picked` in the markup, an SVG containing a picked element is kept uncollapsed, and instead of parent/ancestors the export lists the chain from the element up to the context with box and deviations per level. Slim exports end with a note naming what was removed and how to get the full export.
...
test/                 clipboard-test.html (does a target take image + text from one paste?), iframe-test.html, stitch-test.html (long/wide tables, lasso), record-test.html (hover card, tooltip, panel, long frame), css-context-test.html (cascade, inline style, variables, children, spacing)
```
