...
| `downloads` | On an explicit click (Ctrl+click or menu entry) the rendered HTML of the chosen element is saved as a file into the browser's downloads folder. No other download, no opening of files. |
| `storage` | Stores four settings (subfolder, "Save as" dialog, CSS context, key hints). No user data. |
