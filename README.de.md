...
- **Es sieht nichts, bis Du klickst.** Nur `activeTab` – keine Host-Berechtigung, kein Content-Script auf jeder Seite. Chrome zeigt keine Warnung „kann alle Ihre Daten auf allen Websites lesen". Das zieht zugleich eine Grenze: `activeTab` deckt die Seite selbst ab, nicht die Frames, die eine Seite von anderen Anbietern einbettet (HubSpot- oder Typeform-Formulare, Zahlungsfelder, Einwilligungsbanner). Elemente darin sind nicht wählbar; der Picker sagt das und lässt stattdessen den Frame selbst wählen, mit seiner URL. Wer sie braucht, findet in den Einstellungen eine freiwillige Freigabe, die den Browser um Zugriff auf alle Seiten bittet und dort jederzeit wieder entzogen werden kann.
...
| **R** (schaltet den Aufnahme-Chip scharf, wie gehaltenes Alt oder Strg), dann Klick oder Lasso loslassen – oder Icon-Menü „Interaktion aufnehmen", oder Rechtsklick → „Interaktion hier aufnehmen" | **Interaktions-Aufnahme**, 5 s: hovern, klicken, Tasten drücken – der Picker protokolliert, was die Seite dabei tut (greifende `:hover`-/`:focus`-/`:active`-Regeln, deklarierte Übergänge und Animationen mit Hinweisen wie `transition: all` oder animierte Layout-Eigenschaft, Zustandsdiff der berechneten Stile 450 ms nach dem Eintritt des Zeigers, DOM-Änderungen, laufende Animationen aus `getAnimations()` mit Dauer und Easing, Bildrate mit langen Frames und `long-animation-frame`-Einträgen) und nimmt alle 0,55 s ein Bild auf; die Bilder werden ein Kontaktbogen mit Zeitstempeln und dem Zeiger als blauem Punkt. Für „der Hover ruckelt", „das Pop-up sitzt falsch": ein Video kann ein Sprachmodell nicht lesen, Bilder plus Daten schon. Esc beendet vorzeitig. Strg legt die HTML-Datei des Ziel-Containers dazu; Screenshot und Aufnahme schließen sich aus (das eine scharf schalten schaltet das andere ab) |
| **Shift halten**, solange ein Element umrahmt ist, dann **Shift+↑/↓** oder **Shift+Klick** | **Mehrfachauswahl**: Shift friert das umrahmte Element als Anker ein und zeigt unter der Maus eine gestrichelte Vorschau. Shift+Klick auf ein Geschwister des Ankers wählt den Bereich bis dorthin; Shift+Klick auf ein beliebiges anderes Element nimmt es einzeln dazu (nochmal = wieder weg). Das äußerste gewählte Element, das andere gewählte enthält, wird zum **Kontext**: Screenshot und HTML zeigen den Container, die Selektorzeile nennt die inneren Elemente (alle, auch Zwischenstufen), eine Zeile „Kontext: … (Screenshot und HTML zeigen diesen Container; die Referenz ist die Selektorzeile darüber)" verbindet beides, und je innerem Element nennt eine Zeile seine Lage im Screenshot in Bildpixeln („Im Screenshot: <Selektor> @ x,y · w×h px") – der Screenshot selbst bleibt unangetastet und zeigt das Frontend, wie es ist. Für „hier ist die Section, und ich meine diesen Button". Shift+↑/↓ wandert zum vorherigen/nächsten Geschwister; gibt es keins mehr, springt die Auswahl auf das Elternelement und wandert von dort weiter nach außen. Ein Klick ohne Shift oder **Enter** (auch bei noch gehaltenem Shift) kopiert die Auswahl – immer mit Screenshot über alles, Strg legt eine HTML-Datei mit allen Elementen dazu. Die Selektorzeile wird eine Selektorliste (`#a, #b, #c`) plus „Bereich: 3 benachbarte Elemente, von … bis …" oder „Auswahl: N Elemente (nicht alle benachbart)". Shift nur antippen, ohne etwas zu wählen, gibt den Hover wieder frei |
...
- Optionale Berechtigung: `optional_host_permissions: <all_urls>`, bei der Installation nicht erteilt. Der Schalter „Auch Elemente in eingebetteten Frames fremder Herkunft wählbar machen" in den Einstellungen fordert sie per `permissions.request` an und gibt sie per `permissions.remove` zurück; ohne sie scheitert die Injektion in Frames fremder Herkunft stillschweigend, was der Picker jetzt meldet, statt eine tote Zone zu zeigen (jeder erreichte Frame meldet sich per `postMessage` beim Eltern-Fenster, so weiß der Hauptframe, welche er erreicht hat). Über einem Frame fremder Herkunft bekommt die Seite überhaupt keine Mausereignisse, ein solcher Frame wird deshalb mit Shift+Pfeil von einem Nachbarelement aus erreicht und mit Enter kopiert; hält ein Frame den Fokus, wird er beim Start des Pickers abgegeben, sonst landet jede Modifikatortaste im Frame statt beim Picker.
...
- HTML-Datei: `downloads.download` mit `data:`-URL aus dem Hintergrundskript, Dateiname `JJJJ-MM-TT_HHMM_<host>_<id-oder-klasse>.html`, absoluter Pfad aus `downloads.search`. CSS-Kontext (Leitprinzip: das Ableitbare weg, das Abweichende und Bedeutungstragende bleibt): effektive Werte nur, wo sie vom Initialwert abweichen (ein fester Kern – display, position, Maße, margin, padding – immer), Flex-/Grid-Eigenschaften nur bei passendem `display`; greifende Regeln mit Herkunft (Datei, `inline <style>` als „lädt zuletzt" markiert, Theme/Plugin/Core per Pfad-Heuristik), Spezifität, Media-Query-Zustand und – aus einem Kaskaden-Durchlauf über die getroffenen Deklarationen – welche Eigenschaften eine Regel gegen eine spätere oder stärkere verliert; universelle `*`-Regeln entfallen; `var(--x)` steht mit aufgelöstem Wert; die direkten Kinder mit Box-Modell und ihren Regeln (bei Abstandsaufgaben sitzen die Stellschrauben eine Ebene darunter), eine Abstands-Kette (margin/padding des vorherigen und nächsten Geschwisters, gaps und margins der Kinder), das Elternelement und Vorfahren nur mit Abweichungen (`overflow`, `position`, `transform`, `max-width` …); `::before`/`::after` mit Inhalt (Chevrons, Linien, Icons) und die eigenen Zustandsregeln des Elements (`:hover`, `:focus`, `:active`), die zum Zeitpunkt der Aufnahme nicht aktiv sind; Stylesheets fremder Herkunft sind nicht lesbar und werden als solche genannt. Die Analyse gilt immer den gewählten Elementen: bei einer Mehrfachauswahl bekommt jedes Element einen eigenen Block, und wenn das gezeigte Markup ein Kontext-Container ist (Shift-Bereich mit äußerem Element), sind die gewählten Elemente im Markup mit `data-llment-picked` markiert, ein SVG mit gewähltem Element darin bleibt ungekürzt, und statt Eltern/Vorfahren steht die Kette vom Element hoch bis zum Kontext mit Box und Abweichungen je Stufe. Verschlankte Exporte enden mit einem Hinweis, was entfernt wurde und wie der volle Export zu bekommen ist. Abschaltbar in den Einstellungen.
