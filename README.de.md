...
| **Shift halten**, solange ein Element umrahmt ist, dann **Shift+↑/↓** oder **Shift+Klick** auf ein anderes Geschwister | **Bereich benachbarter Elemente** (mehrere Sections einer Seite): Shift friert das umrahmte Element als Anker ein, Pfeile oder Shift+Klick setzen den Fokus, alles dazwischen ist gewählt und der Rahmen wächst; ein Klick ohne Shift kopiert den Bereich – immer mit Screenshot des ganzen Bereichs, Strg legt eine HTML-Datei mit allen Elementen dazu. Die Selektorzeile wird eine Selektorliste (`#a, #b, #c`) plus Zeile „Bereich: 3 benachbarte Elemente, von … bis …". Shift nur antippen, ohne etwas zu wählen, gibt den Hover wieder frei |
