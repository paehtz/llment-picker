...
**Einstellungen** (Add-on-/Erweiterungsverwaltung → LLMent Picker → Einstellungen): Unterordner im Download-Ordner (Standard `LLMent Picker`), „Speichern unter"-Dialog, Ziel des Screenshots (Zwischenablage und PNG-Datei – Standard –, nur Zwischenablage, nur Datei), CSS-Kontext an/aus, **HTML-Dateien verschlanken** an/aus (Standard an: versteckte Formularfelder, Event-Handler, lange data-Attribute, srcset-Listen, SVG-Pfade, `<style>`-Blöcke, Kommentare, lange `data:`-URIs und wiederholte `<select>`-Optionslisten werden entfernt; die Kopfzeile nennt, was fehlt – eine DNS-Tabelle schrumpfte von 113 auf 52 KB bei gleichem Inhalt), Tastenhinweise an/aus. Erweiterungen dürfen nur in den Download-Ordner des Browsers schreiben; ein freier Zielpfad ist nicht möglich.
...
test/                 clipboard-test.html: prüft, ob ein Ziel Bild + Text aus einem Strg+V übernimmt, iframe-test.html, stitch-test.html (lange/breite Tabellen, Lasso)
```
