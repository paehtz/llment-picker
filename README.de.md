...
| **Shift halten**, solange ein Element umrahmt ist, dann **Shift+↑/↓** oder **Shift+Klick** | **Mehrfachauswahl**: Shift friert das umrahmte Element als Anker ein und zeigt unter der Maus eine gestrichelte Vorschau. Shift+Klick auf ein Geschwister des Ankers wählt den Bereich bis dorthin; Shift+Klick auf ein beliebiges anderes Element nimmt es einzeln dazu (nochmal = wieder weg). Ein Element, das andere gewählte enthält, wird zum **Kontext**: Screenshot und HTML zeigen den Container, die Selektorzeile nennt die inneren Elemente, und eine Zeile „Kontext: … (Screenshot und HTML zeigen diesen Container; gemeint sind die N Element(e) darüber)" verbindet beides – für „hier ist die Section, und ich meine diesen Button". Shift+↑/↓ wandert zum vorherigen/nächsten Geschwister; gibt es keins mehr, springt die Auswahl auf das Elternelement und wandert von dort weiter nach außen. Ein Klick ohne Shift oder **Enter** (auch bei noch gehaltenem Shift) kopiert die Auswahl – immer mit Screenshot über alles, Strg legt eine HTML-Datei mit allen Elementen dazu. Die Selektorzeile wird eine Selektorliste (`#a, #b, #c`) plus „Bereich: 3 benachbarte Elemente, von … bis …" oder „Auswahl: N Elemente (nicht alle benachbart)". Shift nur antippen, ohne etwas zu wählen, gibt den Hover wieder frei |
